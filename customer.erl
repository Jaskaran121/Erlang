        ...
        if
            ...
            true ->
                    ...
                    io:fwrite("Loan requested by customer ~p of amount ~p "
                            "from bank ~p~n",[CustomerName, CurrentLoanRequest, BankName]),
                    receive
                    {Message} ->
                    if 
                        Message == 'approve' ->
                            io:fwrite("Loan approved by bank ~p of amount ~p "
                                "for customer ~p~n",[BankName, CurrentLoanRequest, CustomerName]),
                            startprocessCustomer(MoneyPID, CustomerName,CustomerAmt - CurrentLoanRequest,BankInfo);
                        Message == 'decline' ->
                            io:fwrite("Loan rejected by bank ~p of amount ~p "
                                "for customer ~p~n",[BankName, CurrentLoanRequest, CustomerName]),
                            CurrentBankInfo = lists:delete(CurrentBank, BankInfo),
                            startprocessCustomer(MoneyPID, CustomerName,CustomerAmt, CurrentBankInfo);
                    true ->
                        ok
                    end
        end       
